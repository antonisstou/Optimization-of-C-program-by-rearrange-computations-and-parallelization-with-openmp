...
int main(int argc, char *argv[])
{
	...
	double		s_min;
	double		s_max;
	double		*u, *uplus, *sigma, *omega, *omega1, *temp, *suma;
	double		*sum;
	double		time;
	...
	suma = (double *)calloc(n * n, sizeof(double));
	if (suma == NULL) {
		printf("Could not allocate memory for \"suma\".\n");
		exit(1);
	}

     sum = (double *)calloc(n * n, sizeof(double));
     if (sum == NULL) {
		printf("Could not allocate memory for \"sum\".\n");
		exit(1);
	}
	...
	for (it = 0; it < itime; it++) {
		 ...
		 */
          cblas_dgemv(CblasRowMajor, CblasNoTrans, n, n, 1.0, sigma, n, u, 1, 0.0, sum, 1);
		for (i = 0; i < n; i++) {
			uplus[i] = u[i] + dt * (mu - u[i]);
			uplus[i] += dt * ( sum[i] - suma[i] * u[i] ) / divide;
               if (uplus[i] > uth) {
				uplus[i] = 0.0;
				/*
				 * Calculate omega's.
